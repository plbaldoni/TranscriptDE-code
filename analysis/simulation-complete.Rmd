---
title: "Simulation - Complete report (results presented in the supplementary materials)"
...
- Changes in version 230518:
- With single-end experiments, we now give salmon the true mean/sd of the fragment length distribution. Inclusion of a new simulation scenario with n=10 samples per group.
...
plots <- lapply(seq_len(nrow(dt.scenario)),function(i){
  ...
  figA <- 
    list(plotFDRCurve(x = subsetDT(dt.fdr,scenario,'A'),3000),
         plotPowerBars(x = subsetDT(dt.metrics,scenario,'A'),0.05,3600),
...
capC <- paste(cap,
              'Density histograms for simulations without any differential expression with a maximum of (A) 2 trancripts/gene expressed, (B) 3 trancripts/gene expressed, (C) 4 trancripts/gene expressed, (D) 5 trancripts/gene expressed, and (E) all reference transcripts expressed (averaged over 20 simulations).')  
```
...
```{r simulation-complete_results_panelA,fig.cap=capA,fig.height=13,fig.width=8.5,results='asis'}
...
```{r simulation-complete_results_panelC,fig.cap=capC,fig.height=16,fig.width=8.5,results='asis'}
for(i in seq_len(length(plots))) {
  fig <- plots[[i]]$panelC
  print(fig)
  cat('\n\n<!-- -->\n\n')
