...
Below we generate Figure 3 of the main paper.
...
dt.table.sigma2[,FC.SE := roundPretty(100*(10^(dt.table.sigma2$FC.SE)-1),1)]
dt.table.sigma2[,FC.PE := roundPretty(100*(10^(dt.table.sigma2$FC.PE)-1),1)]

tb <- kbl(dt.table.sigma2,digits = 3,format = 'latex',escape = FALSE,booktabs = TRUE,
          align = c('c','c',rep('c',6)),
          col.names = linebreak(c('Samples per\ngroup','Read Length\n(bp)',
                                  'Increase in overdispersion \n(percentage)','Power','FDR',
                                  'Increase in overdispersion \n(percentage)','Power','FDR'),
                                align = "c")) %>%
