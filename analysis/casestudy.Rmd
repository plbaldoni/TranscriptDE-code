...
Differential isoform expression for genes KRAS (Entrez ID 3845) and PD-L1 (CD274, Entrez ID 29126) has been previously identified in the literature (@yang2018isoform @qu2021pd):

```{r casestudy_res_dte_long_isoforms}
df.bench[df.bench$GeneSymbol %in% c('KRAS','CD274'),]
```
...
ggsave(plot = plot.mao,
       filename = file.path(path.misc,"figure1.pdf"),
...
```{r casestudy_res_real}
# Scaled counts
dte.short.paired.bcv.scaled <- dte.short.paired
dte.short.paired.bcv.scaled$counts <- 
  dte.short.paired.bcv.scaled$counts/dte.short.paired.bcv.scaled$genes$Overdispersion
keep.short.paired.bcv.scaled <- filterByExpr(dte.short.paired.bcv.scaled)
dte.short.paired.bcv.scaled.filtr <- 
  dte.short.paired.bcv.scaled[keep.short.paired.bcv.scaled,, keep.lib.sizes = FALSE]
dte.short.paired.bcv.scaled.filtr <- calcNormFactors(dte.short.paired.bcv.scaled.filtr)
dte.short.paired.bcv.scaled.filtr <- estimateDisp(dte.short.paired.bcv.scaled.filtr,design = design.short.paired)

# Raw counts
dte.short.paired.bcv.raw <- dte.short.paired
keep.short.paired.bcv.raw <- filterByExpr(dte.short.paired.bcv.raw)
dte.short.paired.bcv.raw.filtr <- 
  dte.short.paired.bcv.raw[keep.short.paired.bcv.raw,, keep.lib.sizes = FALSE]
dte.short.paired.bcv.raw.filtr <- calcNormFactors(dte.short.paired.bcv.raw.filtr)
dte.short.paired.bcv.raw.filtr <- estimateDisp(dte.short.paired.bcv.raw.filtr,design = design.short.paired)

plot.bcv.real.raw <- wrap_elements(full = ~foo.bcv(dte.short.paired.bcv.raw.filtr))
plot.bcv.real.scaled <- wrap_elements(full = ~foo.bcv(dte.short.paired.bcv.scaled.filtr))

plot.bcv.real.raw
plot.bcv.real.scaled
```
...
```{r casestudy_res_bcv_sim}
catch.sim <- catchSalmon(list.dirs(file.path(path.sim,'quant-salmon'),recursive = FALSE))
colnames(catch.sim$counts) <- gsub("group|_R1","",basename(colnames(catch.sim$counts)))

dte.sim.raw <- DGEList(counts = catch.sim$counts,
                       genes = catch.sim$annotation,
                       samples = colnames(catch.sim$counts),
                       group = strsplit2(colnames(catch.sim$counts),"_")[,1])
...
dte.sim.raw.filtr <- calcNormFactors(dte.sim.raw.filtr)
dte.sim.raw.filtr <- estimateDisp(dte.sim.raw.filtr,design = design.sim)

dte.sim.scaled <- dte.sim.raw
dte.sim.scaled$counts <- dte.sim.scaled$counts/dte.sim.scaled$genes$Overdispersion

keep.sim.scaled <- filterByExpr(dte.sim.scaled)
dte.sim.scaled.filtr <- dte.sim.scaled[keep.sim.scaled,, keep.lib.sizes = FALSE]
dte.sim.scaled.filtr <- calcNormFactors(dte.sim.scaled.filtr)
dte.sim.scaled.filtr <- estimateDisp(dte.sim.scaled.filtr,design = design.sim)

plot.bcv.sim.raw <- wrap_elements(full = ~foo.bcv(dte.sim.raw.filtr))
plot.bcv.sim.scaled <- wrap_elements(full = ~foo.bcv(dte.sim.scaled.filtr))

plot.bcv.sim.raw
plot.bcv.sim.scaled
```

We put together these four BCV plots into Figure 2 of the main paper.

```{r casestudy_res_bcv_sim_plot}
plot.design.sim <- c(area(1,1),area(1,2),
                     area(2,1),area(2,2))


fig.panel.sim <- wrap_plots(A = plot.bcv.real.raw,
                            B = plot.bcv.real.scaled,
                            C = plot.bcv.sim.raw,
                            D = plot.bcv.sim.scaled,
                            design = plot.design.sim) +
  plot_annotation(tag_levels = 'a') &
  theme(plot.tag = element_text(size = 8))

ggsave(plot = fig.panel.sim,
...
Then, we generate Figure 7 of the main paper below. First we generate panel (a).
...
# Manually export the plot to be combined with wrap_elements
...
ggsave(plot = fig.panel,
       filename = file.path(path.misc,"figure7.pdf"),
       device = 'pdf',width = 10,height = 12,units = 'in',dpi = 300)
```
...
Another set of interesting genes are those that are statistically significant and have trnascripts being DE in a different direction between conditions. Let's take a look at those.

```{r casestudy_res_txgene_switch}
...
length(interestGenes.s)
length(interestGenes.s.cancer)

# Creating heatmat for such genes and their DE transcripts
